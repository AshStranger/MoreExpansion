  ...
  implements Listener
{
  ...
  public void mobSpawn(final CreatureSpawnEvent e)
  {
    Bukkit.getScheduler().scheduleSyncDelayedTask(main.getPlugin(main.class), new Runnable()
    {
      public void run()
      {
        if ((e.getEntity().getCustomName() != null) && (e.getEntity().getCustomName().equals(main.instance.getConfig().getString("mobs.mob1.name").replaceAll("&", "¡ì")))) {
          try
          {
            ...
            hologram.appendTextLine(main.instance.getConfig().getString("mobs.mob1.name").replaceAll("&", "¡ì"));
            new BukkitRunnable()
            {
              public void run()
              {
